...
$(function() {
	...
	var LocationModel = Backbone.Model.extend({
		initialize: function () {
			const polyCoords = this.get('latlngarray');
			...
			polyCoords.forEach(function (element) {
				bounds.extend(element);
			});

			const AdvancedMarkerElement = google.maps.marker?.AdvancedMarkerElement;

			console.log('Logo image URL:', this.get('image'));

			let labelMarker = null;
			let logoMarker = null;

			 if (this.get('image')) {
				const logoUrl = this.get('image');
				const position = bounds.getCenter();

				const LogoOverlay = function(position, imageUrl, map) {
				...
				const logoMarker = new LogoOverlay(position, logoUrl, mapView.getMap());
				this.set({ logoMarker });
			} else {
				// fallback en cas d’absence de logo
				labelMarker = new MapLabel({
	...
	var MapView = Backbone.View.extend({
		...
		showLocations: function(locations) {
			_.each(locations, function(location) {
				const polygon = location.get('marker');
				const label = location.get('label');
				const logo = location.get('logoMarker');
